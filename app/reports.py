...
a = (
      'Referrals, Screening or Assessmnts',
...
def fill_db_student_random_hours(studentid, name):
  ...
  db.session.add(st)
  for i in range(0, 50):
    al = ActivityLog(studentid=studentid, domainid=random.randint(1, len(d)), activityid=random.randint(1, len(a)), locationid=random.randint(1, len(l)), supervisorid=random.randint(1, len(s)), minutes_spent=random.randint(40, 200))
    db.session.add(al)

  db.session.commit()

# format some queries to populate a dictionary the way google charts expects, this passed after using json.dumps() into a jinja macro
# data structure
def build_chart(key, value, core=True):
  title = ''
  if key=='student':
    q = db.session.query(ActivityLog).filter_by(studentid=value).order_by("domainid").all()
    title = db.session.query(Student).filter_by(studentid=value).first().name
  elif key == 'location':
    q = db.session.query(ActivityLog).filter_by(locationid=value).order_by("domainid").all()
    title = db.session.query(Location).filter_by(locationid=value).first().location


  data = {'title': title + ' Core' if core else title + ' Additional', 'graph': [[
    'Category',
  ]], 'total': 0, 'other': 0}
  ...
  for activity in q:
    ...
    total = 0
    for domain in p:
      activityHours[domain[0] if core else domain[0] - 4] = round(domain[1]  / 60, 2)
