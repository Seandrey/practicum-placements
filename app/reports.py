...
def fill_db_student_random_hours(studentid, name):
  ...
  db.session.add(st)
  for i in range(0, 50):
    al = ActivityLog(studentid=studentid, domainid=random.randint(1, len(d)),
                      activityid=random.randint(1, len(a)),
                      locationid=random.randint(1, len(l)),
                      supervisorid=random.randint(1, len(s)),
                      minutes_spent=random.randint(40, 200))
    db.session.add(al)
...
def build_chart(key, value, core=True):
  ...
  domains.append({'role': 'annotation'}) # this is for google charts annotations
  domains.append({'role': 'annotation'}) # this is for google charts annotations
