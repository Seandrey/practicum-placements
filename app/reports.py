...
def fill_db_student_random_hours(studentid, name):
  s = Student(studentid=studentid, name=name)
  db.session.add(s)
  d = (
        'Cardiovascular',
        'Musculoskeletal',
  ...
  for i in range(0, 50):
    a = ActivityLog(studentid=22690264, domainid=random.randint(1, 9), activityid=random.randint(1, 4), minutes_spent=random.randint(40, 200))
    db.session.add(a)
...
# data structure
def build_chart(studentid, core=True):
  q = db.session.query(ActivityLog).filter_by(studentid=studentid).order_by("domainid").all()
  student = db.session.query(Student).filter_by(studentid=studentid).first()

  data = {'title': student.name + ' Core' if core else student.name + ' Additional', 'graph': [[
    'Category',
  ]], 'total': 0, 'other': 0}
...
def get_student_info(studentid):
  data = {}
  data['core'] = build_chart(studentid, True)
  data['additional'] = build_chart(studentid, False)
